...
const VideoList = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            element: row => (
                <>
                <Link to={`/video/edit/${row.id}`} className="btn btn-sm btn-warning me-1"><i className="fa fa-pencil"></i></Link>
    ...
    const requestDeleteApi = () => {
        if (deleteType === 'single') {
            ...
            requestApi(`/videos/${deleteItem}`, 'DELETE', []).then(response => {
                setShowModal(false)
                setRefresh(Date.now())
            ...
            dispatch(actions.controlLoading(true))
            requestApi(`/videos/multiple?ids=${selectedRows.toString()}`, 'DELETE', []).then(response => {
                setShowModal(false)
                setRefresh(Date.now())
                setSelectedRows([])
                dispatch(actions.controlLoading(false))
    ...
    useEffect(() => {
        ...
        requestApi(`/videos${query}`, 'GET', []).then(response => {
            ...
            <main>
                <div className="container-fluid px-4">
                    ...
                    <ol className="breadcrumb mb-4 sticky-top bg-white pb-2">
                        ...
                        <li className="breadcrumb-item active">Danh mục Video</li>
                    </ol>
                    <div className='mb-3'>
